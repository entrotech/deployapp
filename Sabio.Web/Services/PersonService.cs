...
namespace Sabio.Web.Services
{
    public class PersonService : BaseService, IPersonService
    {
        ...
        private SquadBase MapSquad(IDataReader reader, out int personId)
        {
            ...
            sq.MemberId = reader.GetSafeInt32(startingIndex++);
            sq.MemberStatusId = reader.GetSafeInt32(startingIndex++);
        ...
        private Person MapPerson(IDataReader reader)
        {
            ...
            person.ServiceBranchId = reader.GetSafeInt32(startingIndex++);
            person.IsPhonePublic = reader.GetSafeBool(startingIndex++);
            person.IsAddressPublic = reader.GetSafeBool(startingIndex++);
            person.IsEmailPublic = reader.GetSafeBool(startingIndex++);
            person.IsDateOfBirthPublic = reader.GetSafeBool(startingIndex++);
        ...
        public PersonPublic PersonPublic_SelectById(int id)
        {
            PersonPublic pp = new PersonPublic();

            int ord = 0;

            DataProvider.ExecuteCmd(GetConnection, "dbo.PersonPublic_SelectById",
               inputParamMapper: delegate (SqlParameterCollection paramCollection)
               {
                   paramCollection.AddWithValue("@Id", id);
               }
               ...
               {
                   pp.Id = reader.GetSafeInt32(ord++);
                   pp.FirstName = reader.GetSafeString(ord++);
                   pp.LastName = reader.GetSafeString(ord++);
                   pp.City = reader.GetSafeString(ord++);
                   pp.StateProvince = reader.GetSafeString(ord++);
                   pp.PostalCode = reader.GetSafeString(ord++);
                   pp.Country = reader.GetSafeString(ord++);
                   pp.PhotoKey = reader.GetSafeString(ord++);
                   if (!string.IsNullOrWhiteSpace(pp.PhotoKey))
                   {
                   pp.PhotoUrl = SiteConfig.GetUrlFromFileKey(pp.PhotoKey);
                   }
                   pp.PhoneNumber = reader.GetSafeString(ord++);
                   pp.Address1 = reader.GetSafeString(ord++);
